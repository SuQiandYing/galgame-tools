...
def _split_dialog_message_parts(entry):
    """Split physical lines in a tips body while keeping normal dialogue intact."""
    message = '' if entry.get("message") is None else str(entry.get("message"))
    if entry.get("entry_type") == "tip":
...
def _is_tip_call(line):
    """Return whether a call is the three-argument BGI tips renderer."""
    call_name = _extract_call_name(line)
    if not call_name:
        return False
    if call_name in {"grp_::f_200", "f_200"}:
        return True
    prefix, base = _split_qualified_name(call_name)
    return base == "f_200" and prefix == "grp_"


def _parse_tip_call(lines, call_idx):
    """Parse ``grp_::f_200(id, title, body)`` from its stack form."""
    if not _is_tip_call(lines[call_idx]):
        return None

    body_idx = _prev_effective_non_nargs_line(lines, call_idx)
    title_idx = _prev_effective_non_nargs_line(lines, body_idx) if body_idx is not None else None
    id_idx = _prev_effective_non_nargs_line(lines, title_idx) if title_idx is not None else None
    if body_idx is None or title_idx is None or id_idx is None:
        return None

    tip_id = _extract_push_dword(lines[id_idx])
    title = _extract_push_string(lines[title_idx])
    body = _extract_push_string(lines[body_idx])
    if tip_id is None or title is None or body is None:
        return None

    entry = _make_dialog_entry(
        name=title,
        message=body,
        name_line_index=title_idx,
        message_line_index=body_idx,
        is_select=False,
...
def extract_dialog_entries(lines, user_function_names=None):
    ...
    for i, line in enumerate(lines):
        parsed_tip = _parse_tip_call(lines, i)
        if parsed_tip:
            entries.append(parsed_tip)
...
def extract_dialog_json_from_bsd(input_path, output_json, user_function_names=None):
    ...
    payload = []
    for e in entries:
        parts = _split_dialog_message_parts(e)
        for part_index, part in enumerate(parts):
            item = {"message": part}
            if part_index == 0 and e["name"] is not None and e["name"] != "":
                item["name"] = e["name"]
            payload.append(item)
...
def _import_dialog_json_with_split(lines, items, entries, output_bsd):
    expanded = []
    for entry_index, entry in enumerate(entries):
        parts = _split_dialog_message_parts(entry)
        for part_index in range(len(parts)):
            expanded.append((entry_index, entry, part_index, len(parts)))
    if len(items) != len(expanded):
        raise Exception(f"JSON item count mismatch: json={len(items)} bsd={len(expanded)}")

    message_parts = {}
    name_items = {}
    for idx, item in enumerate(items):
        if not isinstance(item, dict):
            raise Exception(f"JSON item {idx} must be an object")
        unknown_keys = set(item.keys()) - {"name", "message"}
        if unknown_keys:
            raise Exception(f"JSON item {idx} contains unsupported fields: {sorted(unknown_keys)}")
        if "message" not in item:
            raise Exception(f"JSON item {idx} is missing message")
        entry_index, _entry, part_index, part_count = expanded[idx]
        if "name" in item and part_index != 0:
            raise Exception(f"JSON item {idx} may only contain name on the first line")
        if "name" in item:
            name_items[entry_index] = '' if item["name"] is None else str(item["name"])
        parts = message_parts.setdefault(entry_index, [None] * part_count)
        parts[part_index] = '' if item["message"] is None else str(item["message"])

    applied = 0
    for entry_index, entry in enumerate(entries):
        parts = message_parts.get(entry_index)
        if not parts or any(part is None for part in parts):
            raise Exception(f"JSON entry {entry_index} is incomplete")
        force_tip_name = entry.get("entry_type") == "tip"
        if (entry_index in name_items or force_tip_name) and entry["name_line_index"] is not None:
            name_text = parts[0] if force_tip_name else name_items[entry_index]
            if entry.get("name_arg_index") is None:
                lines[entry["name_line_index"]] = _replace_push_string(lines[entry["name_line_index"]], name_text)
            else:
            ...
            applied += 1
        message_text = _restore_message_suffix("\n".join(parts), entry.get("message_suffix"))
...
def import_dialog_json_to_bsd(input_bsd, input_json, output_bsd, user_function_names=None):
    ...
    entries = extract_dialog_entries(lines, user_function_names=user_function_names)
    expanded_count = sum(len(_split_dialog_message_parts(entry)) for entry in entries)
    if expanded_count != len(entries) and len(items) == expanded_count:
        return _import_dialog_json_with_split(lines, items, entries, output_bsd)
    ...
    for idx, item in enumerate(items):
        ...
        entry = entries[idx]
        if ("name" in item or entry.get("entry_type") == "tip") and entry["name_line_index"] is not None:
            name_text = (
                ('' if item["name"] is None else str(item["name"]))
                if "name" in item
                else ('' if item["message"] is None else str(item["message"]))
            )
            if entry.get("entry_type") == "tip":
